...
| R | レースの結果の表示(※3) | - |
| C | レースの結果の消去 | - |
| P | スピードガン機能の設定(オフ/3m/5m/10m/15m/20m)(※4) | 5m |
...
ゲート間の距離は、Qキーで設定可能です。距離が大きいほど、高速度における誤差が小さくなります。選択可能なゲート間の距離と、それぞれに適した速度範囲(予想誤差10km/h以内)は、概ね以下のとおりです。
...
- スピードガン機能はレース進行中にのみ動作
- 2つ目のゲートは「最小ラップタイム設定」の時間未満で通過する必要あり
