...
macOSの場合、QuickTimeアプリで、カメラ(受信機)のリストを表示できます。Tiny View Plusを実行する前に、カメラの検出状況を確認しておくと、問題の切り分けがしやすくなります。USBハブを使用していて、映像がコマ落ちするようなら、USBハブ1台あたりの受信機の数を2台以下を目安に減らしてください。
...
| T | ソロ表示のトリミングのオン/オフ | オフ |
| Shift + 1~4 | カメラ1~4の表示のオン/オフ | オン |
| Q | QRコードリーダーの開始/停止(※1) | - |
| B | 背景画像の変更 | アプリ内蔵の画像 |
| F | フルスクリーン表示のオン/オフ | オフ |
| A | ARラップタイマーのモード変更(ノーマル/ルーズ/オフ)(※2) | オン |
| O | ロックオンエフェクトのオン/オフ(※3) | オフ |
| Space | レースの開始/終了 | - |
| V | レースの結果の表示(※4) | - |
...
- (※2)ルーズモードでは、ゲートの外側を通過しても計測対象となります
- (※3)ゲート通過タイムの差が1秒未満の場合に、追跡する側のカメラを拡大表示します
...
以下のマーカーを、4個以上、ゲートの周囲に配置してください。またその際に、マーカーの上辺が、ゲートの中央を向くようにしてください。
...
ラップタイムの計測は、レース中にのみ行われます。同時に2個以上の正しい向きのマーカーを検出した後、画面からマーカーが消えたタイミングでラップタイムが確定します。ただし、最後に映っていたマーカーの向きが正しくない(ゲートのすぐ外側を通った等の)場合は、計測の対象外となります。
